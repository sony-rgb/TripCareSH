...
import { View, Platform, KeyboardAvoidingView, ScrollView, ActivityIndicator, Switch } from 'react-native';
...
export default function ActivityEdit({ navigation, route }) {
  ...
  const { tripItemId } = route.params || {};

  const tripStartDate = selectedTrip ? new Date(selectedTrip.startTime) : null;
  const tripEndDate = selectedTrip ? new Date(selectedTrip.endTime) : null;
  const tripStartStr = tripStartDate ? tripStartDate.toISOString().slice(0, 10) : '1950-01-01';
  const tripEndStr = tripEndDate ? tripEndDate.toISOString().slice(0, 10) : '2050-12-31';
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        const activity = await container.getActivityItemService().getActivityByTripItemId(tripItemId);
        if (!activity) {
        ...
        let loadedStart = activity.startDate ? new Date(activity.startDate).toISOString().slice(0, 10) : '';
        let loadedEnd = activity.endDate ? new Date(activity.endDate).toISOString().slice(0, 10) : '';

        // clamp to trip bounds
        if (loadedStart && loadedStart < tripStartStr) loadedStart = tripStartStr;
        if (loadedStart && loadedStart > tripEndStr) loadedStart = tripEndStr;
        if (loadedEnd && loadedEnd < tripStartStr) loadedEnd = tripStartStr;
        if (loadedEnd && loadedEnd > tripEndStr) loadedEnd = tripEndStr;
        setStartDate(loadedStart);
        setEndDate(loadedEnd);
        
        // Convert 24-hour times to 12-hour format for display
        const start12 = activity.startTime ? convertTo12Hour(activity.startTime) : '';
        const end12 = activity.endTime ? convertTo12Hour(activity.endTime) : '';
        setStartTime(start12);
        setEndTime(end12);
        setIsCompleted(activity.isCompleted ?? false);
      } catch (e: any) {
  ...
  const onSave = async () => {
    ...
    try {
      ...
      if (tripStartDate && start < new Date(tripStartStr)) {
        setErrorMessage('Start date must be within the trip dates.');
        setSaving(false);
        return;
      }
      if (tripEndDate && end > new Date(tripEndStr + 'T23:59:59')) {
        setErrorMessage('End date must be within the trip dates.');
        setSaving(false);
        return;
      }

      const result = await container.getActivityItemService().updateActivityWithTripItem(tripItemId, {
        title: title.trim(),
      ...
      setErrorMessage(error.message || 'Unable to update activity');
    } finally {
      setSaving(false);
    }
  };
  ...
  if (loading) {
    return (
      <View style={[BaseStyle.safeAreaView, { flex: 1, justifyContent: 'center', alignItems: 'center' }]}>
        ...
        <KeyboardAvoidingView
          ...
          <ScrollView contentContainerStyle={styles.container}>
            {selectedTrip ? (
              <View style={styles.tripInfo(colors)}>
                <Text body2 style={{ color: colors.primary, fontWeight: '600' }}>
                  Editing activity in: {selectedTrip.name}
                </Text>
            ...
            <DatePicker
              ...
              minDate={tripStartStr}
              maxDate={tripEndStr}
              current={endDate || startDate || tripStartStr}
              onChange={(date: string) => {
                setEndDate(date);
